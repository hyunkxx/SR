...
CUI_World_HP::CUI_World_HP(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev)
{
}

CUI_World_HP::CUI_World_HP(const CUI_World_HP& rhs)
	: Engine::CGameObject(rhs)
{
}

CUI_World_HP::~CUI_World_HP()
{
}

HRESULT CUI_World_HP::Ready_Object(_float fOrgin_HP)
{
...
_int CUI_World_HP::Update_Object(const _float & fTimeDelta)
{
	Update_HP();
...
void CUI_World_HP::LateUpdate_Object(void)
{
...
void CUI_World_HP::Render_Object(void)
{
...
CUI_World_HP * CUI_World_HP::Create(LPDIRECT3DDEVICE9 pGraphicDev, _float fOrigin_HP)
{
	CUI_World_HP*		pInstance = new CUI_World_HP(pGraphicDev);

	if (FAILED(pInstance->Ready_Object(fOrigin_HP)))
	{
		MSG_BOX("World HP Create Failed");
...
HRESULT CUI_World_HP::Add_Component(void)
{
	CComponent*		pComponent = nullptr;

	pComponent = m_pRcTex = static_cast<CRcTex*>(Clone_Prototype(L"Proto_World_Hp_RcTex"));
	NULL_CHECK_RETURN(m_pRcTex, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Proto_World_Hp_RcTex", pComponent });

	pComponent = m_pTexture = static_cast<CTexture*>(Clone_Prototype(L"Proto_World_Hp_Tex"));
...
void CUI_World_HP::Free(void)
{
	CGameObject::Free();
}

void CUI_World_HP::Update_HP(void)
{
