...
CMiddleTank::CMiddleTank(LPDIRECT3DDEVICE9 pGraphicDev)
...
CMiddleTank::CMiddleTank(const CMiddleTank & rhs)
	: CTankSet(rhs)
{
}
...
void CMiddleTank::Render_Object(void)
{
...
HRESULT CMiddleTank::Ready_Object(void)
{
...
void CMiddleTank::Key_Input(const _float & fTimeDelta)
{
	_vec3	vDir;
	...
	if (!m_bRock)
	{
		...
		{
			m_bPosinShake = true;
			Shoot_Bullet(BULLET_ID::MIDDLE_CANNONBALL);
			m_bReLoad = false;
...
void CMiddleTank::SetUp_OnTerrain(void)
{
	_vec3		vPos;
...
void CMiddleTank::Head_Spin(const _float & fTimeDelta)
{
...
void CMiddleTank::Expect_Hit_Point(const _float & fTimeDelta)
{
	_vec3 Pos, Dir;
...
_vec3 CMiddleTank::PickUp_OnTerrain(void)
{
...
void CMiddleTank::Camera_Change(void)
{
	if (Get_DIKeyState_Custom(DIK_V) == KEY_STATE::TAP)
	{
		if (Engine::Get_Camera_ID() == CAMERA_ID::TANK_CAMERA)
...
void CMiddleTank::Shoot_Smoke(void)
{
...
CMiddleTank * CMiddleTank::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CMiddleTank*		pInstance = new CMiddleTank(pGraphicDev);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CMiddleTank Create Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CMiddleTank::Free(void)
{
...
void CMiddleTank::Update_UI(void)
{
...
void CMiddleTank::Update_Minimap(void)
{
