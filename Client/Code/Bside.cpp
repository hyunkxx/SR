#include "stdafx.h"
#include "..\Header\Bside.h"
#include "Export_Function.h"
#include "UI_Start.h"
#include"RightLocation.h"
CBside::CBside(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev)
{
}

CBside::CBside(const CBside& rhs)
...
HRESULT CBside::Ready_Object(void)
{
...
_int CBside::Update_Object(const _float& fTimeDelta)
{
	CManagement* Temp = CManagement::GetInstance();
	CLayer*      Sour = Temp->Find_Layer(L"Environment");
	CGameObject* Dest;
	CRightLocation* LLTTemp = dynamic_cast<CRightLocation*>(Dest = Sour->Get_GameObject(L"RightLocation"));
...
void CBside::LateUpdate_Object(void)
{
...
void CBside::Render_Object(void)
{
	_matrix	ViewMatrix;
...
CBside* CBside::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CBside*		pInstance = new CBside(pGraphicDev);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CBside Create Failed");
...
HRESULT CBside::Add_Component(void)
{
	...
	pComponent = m_pLogoTexture = static_cast<CTexture*>(Clone_Prototype(L"Proto_Bside"));
	NULL_CHECK_RETURN(m_pTexture, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Proto_Bside", pComponent });
	return S_OK;
}

void CBside::Free(void)
{
	__super::Free();
}
