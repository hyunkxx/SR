...
void CUI_Start::UI_Render_Font(void)
{
	...
	for (_int i = 0; i < NUM_END; ++i)
	{
		...
		if (i == NUM_HELP)
		{
			Render_Font(L"Font_Retro_slim", (m_szDescription[i]).c_str(), &_vec2(200, fY - 20.f), D3DXCOLOR(0.f, 0.f, 1.f, 1.f));
			continue;
		}
		else if (i == NUM_UI || i == NUM_BASE || i == NUM_SKILL || i == NUM_M55 || i == NUM_AH_64A)
		{
			Render_Font(L"Font_Retro_slim", (m_szDescription[i]).c_str(), &_vec2(150.f, fY), D3DXCOLOR(1.f, 0.f, 0.f, 1.f));
			continue;
		}
		else if (i == NUM_1 || i == NUM_2)
		{
			Render_Font(L"Font_Retro_slim", (m_szDescription[i]).c_str(), &_vec2(100.f, fY), D3DXCOLOR(0.f, 0.f, 0.f, 1.f));
			Render_Font(L"Font_Retro_slim", (m_szDescription_Field[i]).c_str(), &_vec2(300.f, fY), D3DXCOLOR(0.f, 0.f, 0.f, 1.f));
			continue;
		}

		Render_Font(L"Font_Retro_slim", (m_szDescription[i]).c_str(), &_vec2(50.f, fY), D3DXCOLOR(0.f, 0.f, 0.f, 1.f));
		Render_Font(L"Font_Retro_slim", (m_szDescription_Field[i]).c_str(), &_vec2(300.f, fY), D3DXCOLOR(0.f, 0.f, 0.f, 1.f));
	}
}
