...
void CSmallTank::LateUpdate_Object(void)
{
	SetUp_OnTerrain();

	if (m_stInfo.fReloadTime > 1.f && !m_bReLoad)
	{
...
void CSmallTank::Render_Object(void)
{
...
HRESULT CSmallTank::Ready_Object(void)
{
...
void CSmallTank::Key_Input(const _float & fTimeDelta)
{
	_vec3	vDir;
...
void CSmallTank::SetUp_OnTerrain(void)
{
	_vec3		vPos;
...
void CSmallTank::Head_Spin(const _float & fTimeDelta)
{
...
void CSmallTank::Expect_Hit_Point(const _float & fTimeDelta)
{
	_vec3 Pos, Dir;
...
void CSmallTank::Camera_Change(void)
{
	if (Get_DIKeyState_Custom(DIK_V) == KEY_STATE::TAP)
	{
	 	if (Engine::Get_Camera_ID() == CAMERA_ID::TANK_CAMERA)
...
void CSmallTank::Posin_Shake(const _float & fTimeDelta)
{
	if (m_bPosinShake)
	{
...
CSmallTank * CSmallTank::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CSmallTank*		pInstance = new CSmallTank(pGraphicDev);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CSmallTank Create Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CSmallTank::Free(void)
{
...
void CSmallTank::Update_Minimap(void)
{
