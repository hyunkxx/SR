#include "..\..\Header\ExplosionEffect.h"
...
CExplosionEffect::CExplosionEffect(LPDIRECT3DDEVICE9 pGraphicDevice, _vec3 vPos)
...
CExplosionEffect::CExplosionEffect(const CExplosionEffect & rhs)
...
CComponent * CExplosionEffect::Clone(void)
{
...
void CExplosionEffect::RenderEffect()
{
	if (!m_bRunning)
		return;
...
CExplosionEffect* CExplosionEffect::Create(LPDIRECT3DDEVICE9 pGraphicDevice, _vec3 vPosition)
{
	CExplosionEffect* pInstance = new CExplosionEffect(pGraphicDevice, vPosition);

	if (!pInstance)
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed CExplosionEffect Create()");
	}
